...
! function(a, b) {
	...
	e.prototype = {
		...
		initialize: function(b, c, d, e) {
			this.mainBox = b.talkVersion ? b : a(b), this.contentBox = c.talkVersion ? c : a(c), this.options = a.extend({
				width: 0
			}, e), this.mainBox.length && this.contentBox.length && (this._createScroll(d), this.resizeScroll(), this._tragScroll(), this._wheelChange(), this._clickScroll())
		},
		...
		_createScroll: function(b) {
			...
			}).appendTo(this.scrollBox), a({
				tag: "span",
				style: a.STYLE_NBODY
			}).appendTo(this.scrollBar), this.scrollBar
		},
		resizeScroll: function() {
			var a = this.mainBox.width(),
				...
				d = this.mainBox.height() - 10 - b,
				e = this.scrollBar.width() || 6;
			this.scrollBox.css({
				position: "absolute",
				background: "#f9f9f9",
				...
				width: Math.max(this.options.width, a - e - c) + "px"
			});
			var f = Math.max(this.contentBox.height(), this.mainBox.height()),
				g = parseInt(d * (d / f)) || 300;
		...
		_tragScroll: function() {
			...
			this.scrollBar.bind("mousedown", function(c) {
				...
				var e = b.mainBox.height(),
					f = b.scrollBar.offset().top - b.scrollBox.offset().top,
					g = c.clientY;
	...
	f.prototype = {
		...
		switchUI: function(a) {
			if(this.contains) switch(a) {
				case this.mode.CON_VIEW_WINDOW:
					this.contains.find(".chat-view-load,.chat-view-message").display(), this.contains.find(".chat-view-window").display(1), this.scroll || (this.scroll = new e(this.chatHistory, this.chatHistory.find("ul"), "chat-view-scrollBar", {
						width: 411
					}));
					break;
		...
		showMessage: function(b, c) {
			...
			if(this.chatHistory.find("li." + g).length && "system" != g) "systembottom" == g && this.chatHistory.find("li." + g).css("visibility", "visible"), d = this.chatHistory.find("li." + g).html(this._getMessageHtml(b, this._contentFilter(c)));
			else if(c) {
				if("system" !== g && "system0" !== g || c.enter && 1 == c.enter || this.chatHistory.find("li." + g).remove(), "first" === g && this.chatHistory.find("ul li").length > 1) h = this.chatHistory.find("li").eq(0);
				else if(i = this.chatHistory.find("li").eq(0 - k), i.indexOfClass("first")) h = null;
				else {
		...
		linkInPageFilter: function(b) {
			var c = this,
				d = this.chatHistory.find("." + b).last().find(".view-history-body").find("span");
			d.each(function(b, d) {
				var e = a(d).attr("src"),
					f = a(d).attr("title") || "自定义标签",
					g = a(d).attr("closebtn") || !0;
		...
		updateMessage: function(b, c, e, f) {
			var g, h = this,
				i = this.chatHistory.find("." + b).last(),
				j = i.find(".view-history-body").last();
			switch(i.find(".view-history-more").bind("click", function() {
				...
				case "8":
					...
					e.from && 1 == e.from && (a.browser.msie && 9 === a.browser.ieversion || o.indexOf("firefox") > -1) ? (l.push(['<video class="ntalker-for-miya-video" style="width:240px;transform:rotate(90deg);-ms-transform:rotate(90deg);margin-left:-30px;margin-top:30px;height:180px"  loop>', '<source src="' + n + '" type="video/mp4">', "</video>", '<span class="ntkf-video-button" style="display:block;width:52px;height:52px;background:url(' + a.button + ') center no-repeat;background-size:100%;position:absolute;top:50%;left:0;right:0;margin:-26px auto 0;"></span>'].join("")), j.css({
						width: "180px",
						height: "240px"
					})) : a.browser.msie && a.browser.ieversion < 9 ? l.push(["<span>", a.lang.cant_play_video, "</span>"].join("")) : (l.push(['<video class="ntalker-for-miya-video" style="width:100%;height:100%;"  loop>', '<source src="' + n + '" type="video/mp4">', "</video>", '<span class="ntkf-video-button" style="display:block;width:52px;height:52px;background:url(' + a.button + ') center no-repeat;background-size:100%;position:absolute;top:50%;left:0;right:0;margin:-26px auto 0;"></span>'].join("")), j.css({
						width: "137px"
					})), j.parent().css({
						padding: "3px"
					}), j.css({
				...
				case "4":
					2 == e.type && 1 == e.emotion ? (a.require(e.sourceurl + "#image", function(b) {
						if(b.error) a.Log("emotion file failure.", 3), e.msgid && h.removeMessage(e.msgid);
						else {
		...
		viewLinkContainer: function(b, c) {
			var d, e = this,
				f = a(c);
			if("kf_9739" == e.siteid) {
				for(var g = !0, h = 0; h < f[0].parentElement.childNodes.length; h++) 3 == f[0].parentElement.childNodes[h].nodeType && (g = !1);
				if(g)
					for(var i = 0; i < a(f[0].parentElement).find("div").length; i++) "" == nTalk(f[0].parentElement).find("div").eq(i)[0].className && nTalk(f[0].parentElement).find("div").eq(i).find("a").css("display", "none")
			}
			...
			f.css({
				...
				var f = a.zoom(c, 75, 75),
					g = (75 - f.height) / 2 + "px " + (75 - f.width) / 2 + "px";
		...
		suggest: function(b, c) {
			var d = this,
				e = this.chatElement.find(".chat-view-hidden-area .chat-view-suggest-list");
				...
				var g = f.replace(d.textEditor.val(), "<span style='color:#ff6f40'>" + d.textEditor.val() + "</span>"),
					h = a({
						...
						d.mode.send({
							msg: c ? f : g,
							botindex: "index"
						}), d.textEditor.val(""), setTimeout(function() {
							e.css("display", "none")
						}, 200)
					});
		...
		_selectSuggest: function(b) {
			var c = this.chatElement.find(".chat-view-suggest-list li"),
				d = 0;
			c.each(function() {
		...
		showInputState: function(c) {
			if(!this._inputStateTimeID || c !== b) {
				c = c ? c : -22;
				var d = this,
					e = this.chatHistory.find(".view-history-body-wait");
		...
		_showFileUpload: function(b, c, d, e) {
			...
			var x = "",
				y = "",
				z = d.oldfile || this.uploadFileName,
				A = d.oldfile || this.uploadFileName,
				B = this.uploadFileSize ? (this.uploadFileSize / 1024).toFixed(2) : d.size ? parseInt(d.size.replace("KB", "")) : "",
				C = /\.[^\.]+$/,
				D = A.toLowerCase().match(C),
				E = [".jpg", ".jpeg", ".png", ".gif", ".bmp", ".pjpeg"],
				F = [".doc", ".docx"],
				G = [".mp3"],
				H = [".txt"];
			...
			var I = 1 == e,
				J = w && v ? " display:none " : "",
				K = I ? " -287px -96px " : " -159px -37px ",
				L = I ? a.lang.news_trans_success : a.lang.news_trans_failure,
				M = I ? a.lang.news_download : "",
			...
			O.css("margin-top", "15px"), O.parent().css("vertical-align", "top"), b.find(".view-history-status-link").last().display(0), b.find(".view-history-status").last().display(0), I || (d.error.maxSize ? d.error.error = a.utils.handleLinks(a.lang.news_trans_failure_size, {
				maxsize: d.error.maxSize / 1048576
			}) : d.error.ext && (d.error.error = a.utils.handleLinks(a.lang.news_trans_failure_type, {
				type: d.error.ext
			})), f.showMessage("system", {
				type: 9,
		...
		_setCursorPosition: function(a, b) {
			if(this._editorStart = b, a.setSelectionRange) a.focus(), a.setSelectionRange(b, b);
		...
		_insertText: function(b) {
			var c = this.textEditor.get(0),
				d = c.value == a.lang.default_textarea_text ? "" : c.value,
		...
		createEvaluation: function(b, c, d, e, f) {
			...
			for(var j, k = 0; k < b.length; k++) "textarea" == b[k].type && (j = g.find("table textarea[name=" + b[k].name + "]").parent(), j.css("position", "relative"), a({
		...
		createEvaluationVersion2: function(b, c) {
			...
			if(!this.evalDialog) {
				...
				this.evalDialog = new a.DialogChat(e, a.extend(a.defaultStyle.evaluateWrap, {
					parent: this.chatElement.get(0)
				})), a.EvaluateEvent.bindEvaluateEvent(), a("[nodeid=submit]").click(function(b) {
		...
		createMessageForm: function(b, c, d, e) {
			var f, g, h = this,
				i = 0,
				j = this.mode.getNewMessageConfig(),
				k = j.message_plan || 1,
				l = j.link_mode || "",
				m = j.specify_link || "";
			if(this.evalDialog && (this.evalDialog.close(), this.evalDialog = null), !this.messageElement.find(".chat-view-message-table table").length) {
				...
				for(var n = 0; n < b.length; n++) b[n] = a.extend(b[n], {
					...
					input: {
						width: "90%",
						height: "textarea" == b[n].type ? "140px" : "auto"
					},
		...
		_fullScreenImage: function(b, c) {
			var d, e = this,
				f = this._createfullScreen(b),
				g = a(b).attr("sourceurl") || b.src,
				h = function() {
			...
			a.Log(this.settingid + ":chatView._fullScreenImage(), src:" + g, 1), a(".view-fullScreen-background").css("opacity", .6), f.click(function(b) {
				...
				for(d in e.imageHash) {
					var h = parseInt(d.substr(0, c.length - 1)),
						i = parseInt(c.substr(0, c.length - 1));
				...
				}), a(".view-prev-picture div,.view-next-picture div").css({
					top: (a(window).height() - 40) / 2 + "px"
				})
			}), f.find("img").attr("src") != g && (f.find("td").css({
				background: "url(" + a.imageloading + ") no-repeat center center"
			}), a.require(g + "#image", function(b) {
				a.Log("nTalk._fullScreenImage() width:" + b.width + ", height:" + b.height);
				var c = a(window).width(),
					d = a(window).height(),
					e = a.zoom(b, c - 100, d);
				f.find("td img").length > 0 && f.find("td img").remove(), f.find("td").append('<img src="' + g + '" width="' + Math.floor(e.width) + '" height="' + Math.floor(e.height) + '" style="' + a.STYLE_NBODY + "margin:0 auto;max-width:" + (c - 100) + "px;max-height:" + d + 'px"/>'), f.find("td img") && f.find("td").css({
					"background-image": ""
				})
			}))
		},
		...
		_bind: function() {
			...
			this.textEditor = this.chatElement.find(".chat-view-window-editor textarea").css({
				...
				a.browser.msie && a.browser.ieversion <= 7 ? a(this).css(a.merge(c, {
					...
					"border-width": "1px"
				})) : a(this).css(a.merge(c, {
					"outline-width": "1px"
				})), a.browser.html5 || a(this).val() == a.lang.default_textarea_text && a(this).val(""), b._listenTextEditor()
			}).bind("blur", function() {
				a.browser.html5 || ("" === a(this).val() && a(this).val(a.lang.default_textarea_text), a(this).val() == a.lang.default_textarea_text && a(this).css({
					color: "#ccc"
				})), a.browser.msie && a.browser.ieversion <= 7 ? a(this).css({
					"border-width": "0px",
				...
				var d = function(c) {
						if(c) {
							a.pasteBase64 = c;
							var d = (a(window).height() - 510) / 2,
								e = d + 480,
								f = (a(window).width() - 640) / 2,
							...
							var h = a(".pastepic-background"),
								i = a(".pastepic-container"),
								j = a(".pastepic-toolbar"),
								k = a(".pastepic-show");
			...
			this.chatElement.find(".chat-view-button,.chat-view-switch-manual").hover(function(b) {
				...
				a.Event.fixEvent(b).stopPropagation(), a(this).css({
		...
		_initFaceGroup: function() {
			...
			this._initFace || (this._initFace = !0, this.chatElement.find(".chat-view-face-tags").length || this.chatElement.find(".chat-view-window-face").append(['<div class="chat-view-face-tags" style="', a.STYLE_NBODY, 'background:#F1F1F1;clear:both;padding:0 10px;height:38px;border-top:1px solid #D4D4D4;"></div>'].join("")), a.each(this.mode.config.faces, function(f, g) {
				var h = "chat-view-face-group-" + f,
					i = "chat-view-face-tag-" + f;
				...
				"img" === c.tagName.toLowerCase() && a(c).css({
					cursor: "pointer",
		...
		_contentFilter: function(b) {
			return "string" != typeof b.msg || /<.*?\>/gi.test(b.msg) ? (1 !== b.type && 7 != b.type || !/<img(.*?)src=([^\s]+)(.*?)>/gi.test(b.msg) || (b.msg = b.msg.replace(/<img(.*?)src=([^\s]+)(.*?)>/gi, '<img class="ntalk-preview" ' + (7 == b.type ? " robotImg='true' " : "") + 'src="' + a.imageloading + '" sourceurl=$2 style="' + a.STYLE_NBODY + '" />')), b) : (b.msg = b.msg.replace(/[\r\n]/gi, " <br>"), b.msg && b.msg.indexOf("xnlink") === -1 && (b.msg = b.msg.replace(/(\s{2})/gi, " {$null}")), b.msg = a.myString(b.msg).linkFilter1(a.STYLE_BODY + "color:#0a8cd2;"), b.msg = b.msg.replace(/\{\$null\}/gi, "&nbsp;&nbsp;"), b.msg = b.msg.replace(/\t/gi, "&nbsp;&nbsp;&nbsp;&nbsp;"), b.msg = a.utils.handleLinks(b.msg, {
				settingid: this.settingid
			}), b.msg = this._faceFilter(b.msg), b = a.extend({
				color: "000000",
				fontsize: "12",
				bold: "false",
				italic: "false",
		...
		_faceFilter: function(b) {
			...
			if(!c || !b) return b;
			for(var e, f = 0; f < c.length; f++)(e = d(c[f])) && (b = b.replace(c[f], '<img src="' + a.sourceURI + "images/faces/" + e + (a.browser.msie6 ? ".gif" : ".png") + '" style="' + a.STYLE_NBODY + 'width:23px;height:23px;margin:0 2px;display:inline;vertical-align:text-bottom;" />'));
			return b
		},
		_image: function() {},
		_file: function() {},
		...
		_viewHistory: function(a) {
			this.mode.viewHistory && (a ? this.chatElement.find(".chat-view-load-history").attr("selected", "selected").css("background-position", "-220px -60px") : this.chatElement.find(".chat-view-load-history").attr("selected", "").css("background-position", "-220px -40px"), this._tempHeader.display(!a), this._chatsHeader.display(a), this._chatsElement.css({
				height: this.chatHistory.height() + "px"
			}).display(a), a && this.mode.viewHistory(this.settingid, this._chatsElement.find("IFRAME.chat-view-float-iframe").get(0)))
		},
		_evaluate: function() {
		...
		_send: function(b) {
			...
			c.length && c != a.lang.default_textarea_text && this.mode.send(c), a.browser.html5 || b !== !0 || this.textEditor.css({
				color: "#ccc"
			}).val(a.lang.default_textarea_text).get(0).focus()
		},
		...
		callChatResize: function(a, b) {
			this.chatHistory.css({
				height: b - 165 + "px"
			}), this.chatElement.find(".chat-view-float-history, .chat-view-float-history iframe").css({
				height: b - 165 + "px"
			}), this.chatElement.find(".chat-view-window-status-info").css("width", a - 40 + "px"), this.evalDialog && this.evalDialog.resize(), this.textEditor.css({
				width: a - 22 + "px"
			}), this.scroll && this.scroll.resizeScroll()
		},
		...
		starLevel: function(b) {
			if(!(a(".nt-evaluation-starlevel").length > 0)) {
				var c = "",
					...
					f = !1,
					g = a.sourceURI + "images/evaluate/fullstar.png",
					h = a.sourceURI + "images/evaluate/emptystar.png",
					j = a.sourceURI + "images/evaluate/halfstar.png";
	...
	g.prototype = {
		...
		update: function(b, c) {
			if(this.title = b ? a.utils.handleLinks(a.lang.toolbar_min_title, {
					destname: b
				}) : a.lang.toolbar_default_text, this.element.find(".ntalk-minimize-title").html(this.title), c && c != a.CON_SINGLE_SESSION) {
				var d, e = this;
		...
		startFlicker: function(c, d) {
			var e = this,
				f = this.count > 99 ? "99+" : this.count,
				g = c ? 1e3 : 500;
			d = d || 0, c === b && this.stopFlicker(!0), a.Log("$.minView.startFlicker(" + a.JSON.toJSONString(arguments) + ") timeid:" + this.timeID, 1), c ? this.element.css({
				"border-color": "#d55f01"
			}).gradient("top", "#ff8803", "#ff7b16") : this.element.css({
				"border-color": "#c8c7c6"
			}).gradient("top", "#e5e5e4", "#f2f3f3").find(".ntalk-minimize-title").html(a.utils.handleLinks(a.lang.toolbar_min_news, {
				count: '<span style="' + a.STYLE_BODY + 'color:#fff;font-weight:bold;">' + f + "</span>"
			})), d >= 7 || (this.timeID = setTimeout(function() {
				d++, e.startFlicker(!c, d)
			}, g))
		},
		stopFlicker: function(b) {
			a.Log("$.minView.stopFlicker()", 1), clearTimeout(this.timeID), this.timeID = null, b || (this.count = 0), this.element.find(".ntalk-minimize-icon").css("background-position", "-98px -38px"), this.element.css({
				"border-color": "#d55f01"
	...
	h.prototype = {
		...
		updateChatTag: function(b, c, d) {
			var e, f = this.header.find(".chat-header-icon"),
				g = this.header.find(".chat-header-name"),
		...
		toggleExpansion: function(b, c) {
			...
			var d = this.header.find(".chat-header-name"),
				e = this.header.find(".chat-header-sign");
			if(a.evaluateStarLevel) e.css("display", "none");
			else {
		...
		updateRightData: function(b, c) {
			...
			return this.settingid = b, c && c.length ? (this._clearTag(), a.each(c, function(a, b) {
				if(b.autoopen)
					for(var a = 0; a < b.autoopen.length; a++) {
						var f = b.autoopen[a].src,
							g = b.autoopen[a].title,
							h = b.autoopen[a].closebutton;
		...
		updataSkin: function(b, c, d) {
			...
			if(c == d)
				...
				} else this.headBody.css({
					background: "url(" + c + ") repeat"
				}), this.rightElement.find(".window-right-head").css({
					background: "url(" + c + ") repeat"
				});
			else this.headBody.gradient("top", c, d), this.rightElement.find(".window-right-head").gradient("top", c, d);
		...
		labelFlicker: function(a, c, d) {
			var e = this,
				f = c ? 1e3 : 500;
			d = d || 0, c === b && this.stopFlicker(a), c ? this.leftContent.find("." + a).css({
				"background-color": "#FE800F"
			}).addClass("talk_flicker") : this.leftContent.find("." + a).css({
				"background-color": "#fafafa"
			}).addClass("talk_flicker"), d >= 7 || (this._flickerTimeID[a] = setTimeout(function() {
				d++, e.labelFlicker(a, !c, d)
			}, f))
		},
		stopFlicker: function(a) {
			clearTimeout(this._flickerTimeID[a]), this._flickerTimeID[a] = null, this.leftBody.find("." + a).css({
				"background-color": "#fafafa"
		...
		_create: function() {
			...
			this._objView = new a.Window(a.extend({
				...
				width: +this.options.width - 2 + "px",
				height: +this.options.height - 1 + "px"
			}).appendTo(this.chatBody), a({
				style: a.STYLE_NBODY + "clear:both;"
			}).appendTo(this.chatBody), this.rightElement = this._objView.rightElement.css({
				width: this.options.rightElementWidth + "px"
			}), this.rightBody = a({
		...
		_onOverChatTag: function(b) {
			for(var c = this;
				"LI" !== c.tagName.toUpperCase();) c = c.parentNode;
			a(c).find(".tag-button-close").css({
				background: "url(" + a.imageicon + ") no-repeat -159px -39px"
			}), a(c).indexOfClass("talk_flicker") || a(c).css({
		...
		_onOutChatTag: function() {
			...
			a(b).find(".tag-button-close").css({
				background: "none"
			}), a(b).indexOfClass("talk_flicker") || a(b).indexOfClass("talk_selected") || a(b).css({
		...
		_callResize: function(a) {
			...
			this.extended.leftElement && (b -= this.options.leftElementWidth), this.extended.rightElement && (b -= this.options.rightElementWidth), this.options.width = b, this.options.height = c - this.options.dropHeight, this.headBody.css("width", a.width - 2 + "px"), this.body.css("width", this.options.width + (this.extended.leftElement ? this.options.leftElementWidth : 0) + "px"), this.leftElement.css({
				...
				width: this.options.width - 2 + "px",
				height: this.options.height - 1 + "px"
			}), this.messageElement = this.chatContainter.find(".chat-view-message"), this.messageElement.css("height", this.options.height - 2 + "px"), this.chatContainter.find(".chat-view-message-table-iframe").css("height", this.options.height - 2 + "px"), this.rightBody.css({
				height: this.options.height - 1 + "px"
			});
			var d = this.options.height - 29;
			this.rightElement.find(".view-right-content").css({
				height: d + "px"
			}), this.rightElement.find(".window-right-content iframe").attr("height", d).css({
				height: d + "px"
		...
		_onCloseChatTag: function(b, c) {
			var d, e = b;
			for(a.Event.fixEvent(c).stopPropagation();
				"LI" !== e.tagName.toUpperCase();) e = e.parentNode;
			a(e).removeClass("talk_selected"), d = e.className.replace(/^\s*|\s*$/g, "") || "", this._manageMode.closeChat(d)
		},
		_getChatPosition: function(b) {
			var c, d;
			if(!b || a.isEmptyObject(b)) this.options.left = Math.max(0, a(window).width() - this.options.width), this.options.top = Math.max(0, a(window).height() - this.options.height - this.options.dropHeight);
			else if(b.rightline && b.width) this.options.left = Math.max(0, (a(window).width() - b.width) / 2 + b.width - this.options.width), this.options.top = Math.max(0, a(window).height() - this.options.height - this.options.dropHeight);
			else if((b.id || b.entryid) && a.isDefined(b.left) && a.isDefined(b.left)) d = b.id || b.entryid || "", d = /(^[#\.])|\s+/gi.exec(d) ? d : "#" + d, a(d).length ? (c = a(d).offset(), b.left = b.left || 0, b.top = b.top || 0, this.options.left = Math.min(c.left - this.options.width + b.left, a(window).width() - this.options.width), this.options.top = Math.min(c.top + b.top, a(window).height() - this.options.height - this.options.dropHeight)) : (this.options.left = Math.max(0, a(window).width() - this.options.width), this.options.top = Math.max(0, a(window).height() - this.options.height - this.options.dropHeight));
			else {
				switch(b.position) {
					case "left-top":
			...
			if(this.rightTags || (this.rightTags = a({
				...
				f = a({
					...
					if("li" === c.tagName.toLowerCase()) {
						var d = a(c).attr("talk_title"),
							e = a(c).attr("talk_content"),
							f = a(c).attr("talk_id");
		...
		_addRightTag: function(b, c, d, e) {
			...
			var f = this.options.height - 28,
				g = this,
				h = a(".window-right-tags").find("div[class]").length,
				i = this.options.rightElementWidth - 1,
				j = Math.floor(i / (h + 1)),
				k = i - j * h,
		...
		_repeatRightTag: function(b) {
			...
			return a(".window-right-content").find("iframe").each(function(d, e) {
				var f = e.src;
				if(f = f.replace(/^http:\/\//, ""), f = f.replace(/\/$/, ""), b = b.replace(/^http:\/\//, ""), b = b.replace(/\/$/, ""), b == f)
					for(var g = e.parentElement.className, h = g.split(" "), d = 0; d < h.length; d++) h[d].indexOf("link_") > -1 && (c = a(".window-right-tags ." + h[d])[0])
			}), c
		},
		_closelinkpage: function(b) {
			...
			var e = a(".window-right-tags").find("div[class]").length,
				f = a(".window-right-tags").width(),
				g = Math.floor(f / e) - 1,
				h = f - (g + 1) * (e - 1) - 1;
	...
	}, "undefined" == typeof a.ntView && (a.ntView = {
		chatView: f,
		minimizeView: g,
		chatManageView: h
	})
...
! function(a, b) {
	...
	a.extend({
		...
		doCapture: function(b) {
			if(b && this.udCapCtl.StartCapture) this.udCapCtl.StartCapture();
			else try {
		...
		addEvent: function(a, b, c) {
			if(this.udCapCtl.attachEvent) this.udCapCtl.attachEvent(a, b);
			else {
				var d = /^function\s?([^\s(]*)/,
					e = b.name || b.toString().match(d)[1] || c,
		...
		loadImageAbnormal: function(b, c) {
			if("ntalk-enterprise-logo" == a(b).attr("data-type")) b.src = a.sourceURI + "images/blank.gif";
			else try {
				var d = a(b).parent().width(),
					e = a(b).parent().height();
				a(b).css({
					margin: "0px"
				}).attr({
		...
		charFilter: function(b) {
			...
			if(a.isArray(b))
				for(c = [], d = 0; d < b.length; d++) "object" == typeof b[d] ? c[d] = a.charFilter(b[d]) : "string" == typeof b[d] ? c[d] = e(b[d]) : c[d] = b[d];
		...
		loadLink: function(b, c) {
			...
			!g || !b || !/^\d+\.\d+\.\d+\.\d+$/gi.test(a.domain) && b.indexOf(a.domain) <= -1 && a.global.pageinchat || (a.Log("nTalk.chatMode.loadLink(" + b + ")"), d = "callback_" + a.randomChar(), e = a.toURI({
				...
				if(a.Log("nTalk.chatMode.loadLink() callback: " + a.JSON.toJSONString(b), 1), b.customer = "", f.view.viewLinkContainer && b.title) {
					if(b.customs && b.customs.length > 0)
						for(var d = 0; d < b.customs.length; d++) b.customs[d] && b.customs[d].name && b.customs[d].content && (b.customer += b.customs[d].name + b.customs[d].content + "<br/>");
		...
		callStat: function(b) {
			var c = new RegExp("^(0|1|2|4|5|6|7|8|18|19|20|21)", "gi"),
				d = new RegExp("^(10|11|12|13|22|23)$", "gi"),
				e = {
					type: "chatjs",
					siteid: this.siteid,
					kfid: this.dest.id || "",
		...
		start: function(b) {
			...
			return !this.config || a.isEmptyObject(this.config) ? void a.Log("chatMode.start():config is null", 3) : (a.Log(this.settingid + ":chatMode.start()"), a.isFunction(this.manageMode.callVerification) && (c = this.manageMode.callVerification(this.settingid, this.config)) ? (c.showMessage("system0", {
				type: 9,
				msg: a.utils.handleLinks(a.lang.system_merge_session, {
		...
		createConnect: function() {
			var b, c = this,
				d = 1 === this.t2dMode ? this.lastSessionID : this.sessionid;
		...
		disconnect: function() {
			...
			"CLOSECHAT" == this.statusConnectTChat ? this.showMessage("system", {
				type: 9,
				msg: a.utils.handleLinks(a.lang.system_end_session, {
					settingid: this.settingid
				})
			}) : "COMPLETE" == this.statusConnectTChat ? 0 !== this.config.enable_auto_disconnect && this.showMessage("system", {
				type: 9,
				msg: a.utils.handleLinks(a.lang.system_auto_disconnect, {
					settingid: this.settingid
				})
			}) : "WAIT" == this.statusConnectTchat && this._clearChangeCsrNum(), this._stopConnectTimeout(), this.view.disableButton(["evaluate", "capture"], !0), this.manageMode.view.updateViewStatus(!0), a.Log(b.settingid + ":chatMode.disconnect()", 1), this.connected = !1, this.statusConnectTChat = "DISCONNECT", this.setDest({
				status: 0
			}), this.chatFlashGoUrl && a.require(this.chatFlashGoUrl + "#rnd", function(c) {
		...
		endSession: function() {
			...
			if(this.manageMode.hash.count() > 1)
				if(a.Log("...............close", 2), this.config && 1 == this.config.enableevaluation && !this._submitRating && this._Evaluable && this._currentView == this.CON_VIEW_WINDOW) {
				...
				} else this.manageMode.closeChat(this.settingid);
			else this.manageMode.close()
		},
		...
		submitMessageForm: function() {
			...
			"kf_9740" === this.siteid ? (c = a.extend(c, {
				...
				m: "Index",
				a: "queryService"
			}), b = this.server.mcenter + "queryservice.php?"), 1 == a.global.message && (c = a.extend(c, {
				opId: a.global.opId
			}));
			var d = a(".chat-view-message-form input[name=msg_email]"),
				e = d.val().replace(/(^\s*)|(\s*$)/g, "");
		...
		startCompress: function(a) {
			this.uploadmsgid = this.showMessage("right", {
				type: "uploadfile" == a ? 4 : 2,
				status: "COMPRESS"
			})
		},
		uploadSuccess: function(b, c) {
			c = a.isObject(c) ? c : a.JSON.parseJSON(c), c = a.protocolFilter(c), this.view.updateMessage(this.uploadmsgid, "uploadfile" == b ? 4 : 2, c), a.Log(this.settingid + ": $.chatMode.uploadSuccess()", 1), this.send(a.extend(c, {
				msg: c
			})), this.uploadmsgid = ""
		},
		...
		postEvaluate: function(b) {
			...
			var d = function() {
					a.Log("evaluate submit complete.", 1), a.browser.mobile ? evMsg = a.lang.system_mobile_evaluation : evMsg = a.utils.handleLinks(a.lang.system_evaluation, {
						evaluation: a.enCut(b.info, 120)
					}), c.showMessage("info", {
						type: 9,
						msg: evMsg
					})
		...
		sendFirstMessage: function() {
			if(this.requestRobot && 0 !== this.config.enable_robotgreeting && 1 == a.server.robot) {
				...
				var b = this.config.greet_detail ? this.config.greet_detail : a.utils.handleLinks(a.lang.system_first_news, {
		...
		send: function(b, c) {
			...
			if(b = "string" == typeof b ? a.extend({}, this.defData, e, {
					msg: b.replace(/</gi, "&lt;").replace(/>/gi, "&gt;")
				}) : a.extend({}, this.defData, e, b), !this.connected) return /FAILURE|QUEUE/i.test(this.statusConnectTChat) || (a.Log("connected:" + this.connected + ", statusConnectTChat:" + this.statusConnectTChat + ", start", 1),
				...
				if(c.logo && (c.logo = a.protocolFilter(c.logo)), c.url && (c.url = a.protocolFilter(c.url)), c.sourceurl && (c.sourceurl = a.protocolFilter(c.sourceurl)), c.mp3 && (c.mp3 = a.protocolFilter(c.mp3)), c.msg && "string" == typeof c.msg && c.msg.indexOf("xnlink") > -1 && (c.xnlink = !0), c.systype) {
					...
					if(b = "left", "2" === c.systype || "5" === c.systype) {
						...
						a.each(h, function(b, d) {
							if("message" == b) c.msg = a.utils.handleLinks(c.msg.replace(d, "[link message={$settingid} source=2]" + d + "[/link]"));
							else {
								d = d.split(",");
		...
		_sendGoodsinfo: function() {
			...
			if(this.options.itemid) {
				if(this.callStat("20"), b = this.server.mcenter + "/goodsinfo/api.php?" + a.toURI({
						...
						msg: {
							msgtype: 5,
							productInfoURL: b + "&type=2&ts=" + a.getTime()
						}
		...
		initConfig: function() {
			var b, c;
			return !this.options.config || a.isEmptyObject(this.options.config) ? void this.switchUI(this.CON_VIEW_ERROR, "LOAD_FAIED") : (this.switchUI(this.CON_VIEW_WINDOW, "LOAD_COMPLETE"), this.config = a.extend({
				settingid: this.settingid
			}, this.options.config), this.options.config.service ? this.server = a.extend({}, a.server, a.protocolFilter(this.options.config.service)) : (a.Log("config file version error.", 3), this.server = a.extend({}, a.server, {
				tchatserver: "",
				tchatgoserver: "",
				filetranserver: ""
				...
				name: a.utils.handleLinks(a.lang.system_title_news, {
					name: this.config.name || ""
				}),
				status: 0
		...
		audioUpload: function(b, c) {
			...
			if("uploading" === b.status) this.uploadingid[c] || (this.uploadingid[c] = "temp", this.uploadingid[c] = this.showMessage("right", {
				type: 6,
				msg: "uploading"
			})), d = (b.event.loaded / b.event.total * 100).toFixed(2), this.uploadingid[c] && "temp" != this.uploadingid[c] && this.view.audioProgress(this.uploadingid[c], d);
			else if("success" === b.status) var h = setInterval(function() {
				if(g.uploadingid[c] && "temp" != g.uploadingid[c]) {
					...
					f.type = 6, f.sourceurl = f.url, f.url = f.mp3, f.duration = f.length, delete f.mp3, g.view.updateMessage(g.uploadingid[c], 6, f), a.Log("audioUpload:" + a.JSON.toJSONString(f), 2), g.send(a.extend(f, {
						msg: f
					})), g.view.showAudioResult(g.uploadingid[c]), g.uploadingid[c] = ""
				}
			}, 200);
			else "error" === b.status ? g.view.showAudioResult(g.uploadingid[c]) : a.Log(b, 3)
		},
		_initChatConfig: function() {
			var b, c = this,
				d = [];
			if(a.isDefined(this.config.message_skin) && ("chat/2" == this.config.message_skin || "" === this.config.message_skin || this.config.message_skin.indexOf("|") > -1)) this.config.message_skin = this.config.message_skin ? this.config.message_skin : "#2c2c2e|#474749", this.config.startColor = this.config.message_skin.substr(0, this.config.message_skin.indexOf("|")), this.config.endColor = this.config.message_skin.substr(this.config.message_skin.indexOf("|") + 1);
			else {
				var e = {
					"chat/1": "#4297e0",
					"chat/3": "#575757",
			...
			this.config.faces = this.config.faces || [], b = {
				...
				switch(b) {
					case "about":
						...
						break;
					case "faq":
						c.config.faqlist && c.config.faqlist.length && d.push(a.extend(e, {
							data: c.config.faqlist || []
						}));
						break;
					case "linkinpage":
						d.push(e);
		...
		_abnormal: function(b) {
			var c = a.utils.handleLinks(a.lang.system_abnormal, {
				settingid: this.settingid
			});
		...
		_failure: function(b) {
			var c = a.utils.handleLinks(a.lang.system_failure, {
				settingid: this.settingid
			});
		...
		_offline: function() {
			var b = a.utils.handleLinks(a.lang.system_offline, {
				destname: this.dest.name,
				settingid: this.settingid
			});
		...
		_ready: function(b, c) {
			a.Log(this.settingid + "::chatMode._ready()", 1), this.connect && this.connect.stopSwitchConnect(), this.statusConnectTChat = "READY", "zh_cn" !== a.lang.language.toLowerCase() && (this.debug && a.Log(this.settingid + ":chat.connect.setTextStyle"), this.connect && this.connect.setTextStyle(a.JSON.toJSONString({
				fontsize: 20
			}))), this.callStat("4")
		},
		_connectSuccess: function(b) {
			...
			b && (c = "string" == typeof b ? a.JSON.parseJSON(b) : b, this.setUser({
				...
				type: 9,
				msg: a.utils.handleLinks(a.lang.system_switch_session, {
					destname: this.dest.name
				})
		...
		_connectException: function() {
			a.Log(this.settingid + ":chatMode._connectException()"), this.connected = !1, this.statusConnectTChat = "FAILURE", this.showMessage("system", {
				type: 9,
				msg: a.utils.handleLinks(a.lang.system_connect_wait, {
					settingid: this.settingid
				})
			})
		},
		...
		notifyUserList: function(b) {
			...
			for(var d = [], e = 0; e < b.length; e++) a.base.checkID(b[e].userid) === a.CON_CUSTOMER_ID && (d.push(b[e]), this.addDestList({
				id: b[e].userid || "",
		...
		userEnter: function(b) {
			var c, d = a.lang.system_add_session,
				e = !0;
			try {
				c = a.JSON.parseJSON(b)
			} catch(f) {
			...
			if(a.base.checkID(c.userid) == a.CON_CUSTOMER_ID && 0 !== this.userList.length) {
				...
				e && (this.userList.push(c), this.userNumber = this.userList.length), this.userList.length > 1 && this._clearChangeCsrNum(), a.Log(this.settingid + ":[" + this.userList.length + "]chatMode.userEnter(" + b + ")"), this.addDestList({
					...
					type: 9,
					msg: a.utils.handleLinks(d, {
						destname: c ? c.externalname || c.nickname || "" : this.dest.name
					}),
					enter: 1
				}))
			}
		...
		userLeave: function(b) {
			...
			if(c && !a.isEmptyObject(c)) {
				...
				for(var d = [], e = 0; e < this.userList.length; e++) this.userList[e].userid != b && d.push(this.userList[e]);
				...
				this.setDest({
					...
					type: 9,
					msg: a.utils.handleLinks(a.lang.system_go_away_session, {
						destname: c.name
					}),
					enter: 1
				})
		...
		addDestList: function(b) {
			...
			if(b && !a.isEmptyObject(b) && (b.id || b.userid)) return d = b.userid || b.id, e = b.externalname || b.nickname || b.username || b.name, f = b.usericon || b.logo || "", a.Log("add or update dest info:" + a.JSON.toJSONString(b), 2), this.hashDest.contains(d) ? (c = a.extend({}, this.hashDest.items(b.id), {
				...
				name: e,
				logo: f
			}), this.hashDest.items(c.id, c)) : (c = {
				id: d,
				name: e,
				logo: f
			}, this.hashDest.add(c.id, c)), c
		},
		getMsgId: function(b) {
		...
		_formatEvaluationData: function(b) {
			...
			if(b = this._filterNullChar(b), 2 == this.config.evaluateVersion ? e.msg = a.extend({
					msgtype: 3
				}, {
					newevaluate: b
				}) : e.msg = a.extend({
				...
				for(var f in b) b[f] && b[f].value && !a.isFunction(b[f]) && b.hasOwnProperty(f) && (c += "string" == typeof b[f].value ? b[f].value + "; " : b[f].value.text + "; ");
			return a.Log("submitData::" + a.JSON.toJSONString(e)), {
				data: this._toEvaluateXML(e),
				info: a.enCut(c, 50)
			}
		},
		...
		open: function(b, c, d, e, f, g, h, i) {
			...
			var j = this;
			if(this.htmlSID = a.getTime(2), this.settingid = b, this.destid = c || "", this.itemid = d, this.itemparam = e, this.sellerid = f, this.single = h || (this.destid ? 1 : 0), this.manual = i || "0", this.clearHistoryPageCount(), this.view && this.objMinView && this.objMinView.remove(), this.createClientID(), this.hash.contains(this.settingid)) this.hash.items(this.settingid) && (a.Log("$.chatManage.switchChat(" + this.settingid + ")", 1), this.chat = this.hash.items(this.settingid), this.chat.selected || this.switchChat(this.settingid));
			else {
		...
		loadWapView: function(b, c) {
			...
			b.wapTheme && "1" !== b.wapTheme.layout && (d = "chat.view.wap.theme" + b.wapTheme.layout + ".js"), a.require({
				view: d + a.baseExt
			}, function() {
				c.call()
			})
		...
		initChatManage: function(c, d) {
			var e, f, g = this,
				h = {};
			if(this.view || ("kf_9740" == a.global.siteid ? h.position = {
					...
					b && b.minimize()
				}), d && 1 == d.autoconnect) a.Log("autoconnect:1"), c = !0;
			else if(d && d.autoconnect == -1) c = !1;
			else if(e = a.store.get(a.base.CON_LOCAL_FIX + this.settingid)) {
				var i = a.getTime() - e;
				i < 18e5 && (c = !0)
			}
			try {
		...
		beforeunload: function(b) {
			if(0 !== this.hash.count() && (this.chat.connected && this.chat._sendNum > 0 && 0 !== this.chat.config.sessioncarry ? (a.cache.set("carry_sid", this.chat.settingid), a.cache.set("carry_did", this.chat.dest.id)) : (a.cache.set("carry_sid", ""), a.cache.set("carry_did", "")), !a.global.pageinchat && !a.browser.mobile))
				if(this.chat && this.chat.config && 1 == this.chat.config.enableevaluation && this.chat._Evaluable && !this.chat._submitRating) {
		...
		get: function(b, c) {
			...
			if(c && a.base.checkID(c) == a.CON_CUSTOMER_ID)
				for(var d in this.hash.hashTable) {
		...
		chatReady: function() {
			a.waitMessage || (a.waitMessage = new a.HASH, a.waitMessage.verificationAdd = function(a, b) {
				...
				msg: {
					msgtype: 7,
					param: a.global.erpparam + "|lang=" + (a.global.lang || a.language)
				}
