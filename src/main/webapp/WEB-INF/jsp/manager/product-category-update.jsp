...
<div class="type_style">
    <div class="type_title">修改产品分类</div>
    <div class="type_content">
        <%--<div class="Operate_btn">--%>
            <%--<a class="btn  btn-warning"><i class="icon-edit align-top bigger-125"></i>新增商品类型</a>--%>

        <%--</div>--%>
        <form action="<%=basePath%>part/update" method="post" class="form form-horizontal" id="form-user-add">
            <input type="hidden" value="${part.id}" name="id">
            <div class="Operate_cont clearfix">
                <label class="form-label"><span class="c-red">中文&nbsp;&nbsp;&nbsp;&nbsp;</span>分类名称：</label>
                <div class="formControls ">
                    <input type="text" class="input-text" value="${part.name}" placeholder="" name="name">
                </div>
            </div>
            <div class="Operate_cont clearfix">
                <label class="form-label"><span class="c-red">日文&nbsp;&nbsp;&nbsp;&nbsp;</span>分类名称：</label>
                <div class="formControls ">
                    <input type="text" class="input-text1" value="${part.nameJ}" placeholder="" name="nameJ">
                </div>
            </div>
            <div class="Operate_cont clearfix">
                <label class="form-label">优先顺序：</label>
                <div class="formControls ">
                    <input type="text" class="input-text" value="${part.priority}" placeholder="" id="user-name" name="priority">
                </div>
            ...
            <div class="Operate_cont clearfix">
                ...
                <select id="city1" name="belong" onchange="" style="margin-left:10px;width: 150px;">
                    <c:if test="${part.belong == null}">
                        <option value="-1">一级父分类</option>
                    </c:if>
                    <c:if test="${part.belong != null}">
                        <c:if test="${child!=null}">
                            <option value="${child.belong}">${child.fname}</option>
                        </c:if>
                        <c:if test="${child==null}">
                            <option value="${part.belong}">${part.fname}</option>
                        </c:if>
                        <option value="-1">一级父分类</option>
                    </c:if>
